...
const { loadContact, findContact, addContact, checkDuplicate, deleteContact, updateContacts, } = require('./contacts');
const port = 3000
...
// delete masal
app.post('/contact/hapus', (req, res)=>{
  const {nama} = req.body
  console.log(nama)
  if(Array.isArray(nama)){
    nama.forEach(kontak => {
      deleteContact(kontak)
      res.redirect('/contact')
...
if(!errors.isEmpty()) {
  ...
  // // set massage
  req.flash('msg','Data hasbeen updated')
  res.redirect('/contact')
}
})
